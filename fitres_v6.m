...
%%
function  [P_av, Pr_av,Pinf_av,Pn_av,Tn_av,Sn_av,fita_av,fitb_av,rsq_av,dp,nn]=fitres_v6(pulse,sampling_rate,version) 
%%
...
%     end
    ...
    % all data
    T_av=length(p_av)/sampling_rate; % Duration of single beat
...
%% Trail with pressure from start of diastole
%     a = find(diff(periph_pulse')<0,1,'last'); 
%     b = periph_pulse(1:a);
%     plot(b);
...
% (a) exclude diff (p)>0 in diastole
    ...
    P_av=p_av(1:cut);   % Pressure starting at diastolic pressure.
    ...
    if version == 14
        [Pr_av,fita_av,fitb_av,Pinf_av,Tn_av,Pn_av,dp,nn]=kreservoir_v14(P_av,T_av, sampling_rate);
    elseif version == 16
        [Pr_av,fita_av,fitb_av,Pinf_av,Tn_av,Pn_av,dp,nn]=kreservoir_v16(P_av,T_av, sampling_rate);
    end
