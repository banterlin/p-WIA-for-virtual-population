...
%     fprintf("r^2 of diastolic fit is %f \n",rsq_av);
    ...
    plot(TimeC,di);title('Wave intensity from pressure-only method');
    ...
    proc_var{record_no,1}= ID;         % filename
    proc_var{record_no,2}=dippks(1);       % W1 intensity
    proc_var{record_no,3}=dipt(1);         % W1 time
    proc_var{record_no,4}=diparea(1);      % W1 area
    proc_var{record_no,5}=dimpks;          % W-1 intensity
    proc_var{record_no,6}=dimt;            % W-1 time
    proc_var{record_no,7}=dimarea;         % W-1 area
    proc_var{record_no,8}=dippks(2);       % W2 intensity
    proc_var{record_no,9}=dipt(2);         % W2 time
    proc_var{record_no,10}=diparea(2);     % W2 area
    proc_var{record_no,11}=max_prld;       % max Pres-diastolic, mmHg
    proc_var{record_no,12} = max_pxs;      % Max Excess Pressure
    proc_var{record_no,13}=max_txs/sampling_rate; % time of max excess P, sec
    proc_var{record_no,14}=sum(Pxs)/sampling_rate; % Integral excess pressure, mmHg.s
    proc_var{record_no,15}=sum(Pr_av)/sampling_rate; % Integral Reservior pressure, mmHg.s
    proc_var{record_no,16}=Tn_av;           % Time of end systole by max -dp/dt, sec
    proc_var{record_no,17}=Pn_av;           % P at end systole by max -dp/dt, mmHg
    proc_var{record_no,18} = SBP;           % Systolic Blood Pressure
    proc_var{record_no,19} = DBP;           % Diastolic Blood Pressure
    proc_var{record_no,20} = AP;            % Augmentation Pressure
    proc_var{record_no,21} = AIx;            % Augmentation Index
    proc_var{record_no,22}=Pinf_av;         % P infinity, mmHg
    proc_var{record_no,23}=fita_av;         % rate constant A (ka), 1/sec
    proc_var{record_no,24}=fitb_av;         % rate constant B (kb), 1/sec
    proc_var{record_no,25}=wri;             % True Wave Reflection index
    proc_var{record_no,26}=rsq_av;          % R2 for diastolic fit
    proc_var{record_no,27}=Pb_Pf;           % Pb/Pf (ratio of backward to forward pressure - also know as reflection magnitude. RM
    proc_var{record_no,28}=RI;              % Reflection index (RI) calculated as Pb/(Pb+Pf)
...
header = {'patid' 'wf1_mag' 'wf1_time' 'wf1_area' 'wb_mag' 'wb_time'...
    'wb_area','wf2_mag','wf2_time','wf2_area', 'Pres_endsys'...
    'Pxs_max','Pxs_max_time','Int_Pxs','Int_Pres','Tn_av','P_endsys','SBP','DBP','AP','AIx' ...
    'Pinf','fita','fitb','wri','rsq_av','re_pb_pf','RI'}; % header
