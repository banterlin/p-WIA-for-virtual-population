...
%       optimised single pressure waveform detection --> imporved results
...
%     central_signal= central_signal(~isnan(central_signal));
    ...
    [dixs,wrixs,diplocsxs,dippksxs,dimlocsxs,dimpksxs,diptxs] = WIA(Npoly,Frame,cpwia,cuxwia,sampling_rate);
...
%     rhocxs=max(Pxs)*mmHgPa/1000; % fixed units (m/s)
    ...
    [di,wri,diplocs,dippks,dimlocs,dimpks,dipt] = WIA(Npoly,Frame,p,u,sampling_rate);
...
%     proc_var{record_no,9}=max_prld;         % max Pres-diastolic, mmHg
    ...
    proc_var{record_no,19}=max_velocity;              % Reflection index (RI) calculated as Pb/(Pb+Pf)
...
%     proc_var{record_no,46}=rhocxs;              % rhoc estimation for reservoir analysis
    proc_var{record_no,20}=wri;            % WRI for conventional method
    proc_var{record_no,21}=wrixs;          % WRI  for excess pressure
...
%     proc_var{record_no,25}=version;         % version of main used
    proc_vac{record_no,25} = kres_v;            % Version of reservior analysis used
...
xlsfile='C:\Spdata\results\resdata.xlsx';
header = {'re_file' 'dippks1' 'dimpks' 'dippks2' 'dippksxs1' 'dimpksxs'...
    'dippksxs2'	're_sam_rate', 're_intxsp','re_maxxsp'...
    're_tmaxxsp' 're_tn' 're_pinf' 're_pn' 're_fita' 're_fitb'	're_rsq' ...
    're_pb_pf', 're_max_v', 're_wri','re_wrixs','corr_Pxs', 'corr_dI', ... 
    'DTW_dist','kres_v'}; % header
