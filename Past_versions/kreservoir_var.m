%% kreservoir_variable  -  calculate reservoir pressure from pressure waveform
...
%  v.var (variable notch timing)--> using variable tn_av to compute for the maximum corrdI
%%
function [Pr,A,B,Pinf,Tn,Pn]=kreservoir_var(P,Tb, sampling_rate,Tn_av)
...
%           Tn   - detected time of the dicrotic notch
...
%     dp=fsg721(p);
%     [~,nn]=min(dp); 
%     nn = round(nn + nn/5);       % Using last two thrid of diastolic 
    nn=Tn_av;       %% potential to add a potential variable input nn, for
%     when r2 score is lower than our accpeatable allowence
    tn=t(nn);       %Dirotic notch time
