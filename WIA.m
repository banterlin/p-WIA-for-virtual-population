...
function [di,wri,diplocs,dippks,dimlocs,dimpks,dipt,dimt,diparea,dimarea] = WIA(Npoly,Frame,P,U,sampling_rate)
...
[dippks(1),diplocs(1), dipw(1)]=findpeaks(di(1:lsys)+10, 'NPeaks',1,'MinPeakHeight',minpeak); % find 1st dI+ peaks (Wf1)
[dimpks,dimlocs,dimw]=findpeaks(-di(1:lsys), 'NPeaks',1,'MinPeakHeight',0.7*max(-di)); % find one dI- peaks
[dippks(2),diplocs(2), dipw(2)]=findpeaks(flip(di(1:lsys)), 'NPeaks',1,'MinPeakHeight',minpeak); % find 2nd dI+ peaks (Wf2) by flipping the data and running findpeak in the other direction
diplocs(2)=lsys-diplocs(2)+1; % correct location for the flipping
%% check peaks
